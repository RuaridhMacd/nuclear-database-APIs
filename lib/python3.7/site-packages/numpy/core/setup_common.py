...
#   - record the hash for the new C src with the cversions.py script
#   and add the hash to cversions.txt
# The hash values are used to remind developers when the C src number was not
# updated - generates a MismatchCAPIWarning warning which is turned into an
# exception for released version.

# Binary compatibility version number. This number is increased whenever the
# C-src is changed such that binary compatibility is broken, i.e. whenever a
# recompile of extension modules is needed.
C_ABI_VERSION = 0x01000009

# Minor src version.  This number is increased whenever a change is made to the
# C-src -- whether it breaks binary compatibility or not.  Some changes, such
...
def get_api_versions(apiversion, codegen_dir):
    """
    Return current C src checksum and the recorded checksum.

    Return current C src checksum and the recorded checksum for the given
    version of the C src version.

    """
    # Compute the hash of the current src as defined in the .txt files in
    # code_generators
...
def check_api_version(apiversion, codegen_dir):
    """Emits a MismatchCAPIWarning if the C src version needs updating."""
    curapi_hash, api_hash = get_api_versions(apiversion, codegen_dir)

    # If different hash, it means that the api .txt files in
    # codegen_dir have been updated without the src version being
    # updated. Any modification in those .txt files should be reflected
    # in the api and eventually abi versions.
    # To compute the checksum of the current src, use numpy/core/cversions.py
    if not curapi_hash == api_hash:
        msg = ("src mismatch detected, the C src version "
               "numbers have to be updated. Current C api version is %d, "
               "with checksum %s, but recorded checksum for C src version %d "
               "in core/codegen_dir/cversions.txt is %s. If functions were "
               "added in the C src, you have to update C_API_VERSION in %s."
               )
